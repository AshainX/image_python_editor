...
#"""****Triggered Function for Main Menu Options****"""
    ...
    def crop_action(self):
        ...
        self.canvas.bind("<ButtonRelease>", self.end_crop)   #crop done on leaving the click

    def start_crop(self, event):
        self.crop_start_x = event.x     #start crop points(x,y)
        self.crop_start_y = event.y

    def crop(self, event):
        if self.rectangle_id:                          #crop free on square nd rect
    ...
    def end_text_crop(self, event):
        if self.crop_start_x <= self.crop_end_x and self.crop_start_y <= self.crop_end_y:
            start_x = int(self.crop_start_x * self.ratio)
            start_y = int(self.crop_start_y * self.ratio)   #y-x starting and end point
            end_x = int(self.crop_end_x * self.ratio)        # startx<=endx and starty<=endy
            end_y = int(self.crop_end_y * self.ratio)
        elif self.crop_start_x > self.crop_end_x and self.crop_start_y <= self.crop_end_y:
            start_x = int(self.crop_end_x * self.ratio)
            start_y = int(self.crop_start_y * self.ratio)    #x-y starting point 
            end_x = int(self.crop_start_x * self.ratio)      # startx>endx and starty<=endy
            end_y = int(self.crop_end_y * self.ratio)
        elif self.crop_start_x <= self.crop_end_x and self.crop_start_y > self.crop_end_y:
            start_x = int(self.crop_start_x * self.ratio)
            start_y = int(self.crop_end_y * self.ratio)      #x-x 
            end_x = int(self.crop_end_x * self.ratio)        # startx<=endx and starty>endy
            end_y = int(self.crop_start_y * self.ratio)
        else:
            start_x = int(self.crop_end_x * self.ratio)
...
#"""**Filter Menu Options***"""
    ...
    def filter_action(self):                       #filter sections 
        self.refresh_side_frame()
        ttk.Button(
